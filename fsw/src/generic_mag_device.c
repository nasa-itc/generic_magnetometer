...
int32_t GENERIC_MAG_ReadData(int32_t handle, uint8_t* read_data, uint8_t data_length)
{
    ...
    while((bytes_available < data_length) && (ms_timeout_counter < GENERIC_MAG_CFG_MS_TIMEOUT))
    {
        ms_timeout_counter++;
        OS_TaskDelay(1);
        bytes_available = uart_bytes_available(handle);
    ...
    if (ms_timeout_counter < GENERIC_MAG_CFG_MS_TIMEOUT)
    {
        ...
        if (bytes != bytes_available)
        {
                ...
                OS_printf("  GENERIC_MAG_ReadData: Bytes read != to requested! \n");
            #endif
            status = OS_ERROR;
        } /* uart_read */
    }
...
*/
int32_t GENERIC_MAG_CommandDevice(int32_t handle, uint8_t cmd_code, uint32_t payload)
{
    int32_t status = OS_SUCCESS;
    int32_t bytes = 0;
    uint8_t write_data[GENERIC_MAG_DEVICE_CMD_SIZE] = {0};
    uint8_t read_data[GENERIC_MAG_DEVICE_DATA_SIZE] = {0};

    payload = CFE_MAKE_BIG32(payload);

    /* Prepare command */
    write_data[0] = GENERIC_MAG_DEVICE_HDR_0;
    write_data[1] = GENERIC_MAG_DEVICE_HDR_1;
    write_data[2] = cmd_code;
    write_data[3] = payload >> 24;
    write_data[4] = payload >> 16;
    write_data[5] = payload >> 8;
    write_data[6] = payload;
    write_data[7] = GENERIC_MAG_DEVICE_TRAILER_0;
    write_data[8] = GENERIC_MAG_DEVICE_TRAILER_1;
    ...
    {
        /* Write data */
        bytes = uart_write_port(handle, write_data, GENERIC_MAG_DEVICE_CMD_SIZE);
        #ifdef GENERIC_MAG_CFG_DEBUG
            OS_printf("  GENERIC_MAG_CommandDevice[%d] = ", bytes);
            for (uint32_t i = 0; i < GENERIC_MAG_DEVICE_CMD_SIZE; i++)
            {
                OS_printf("%02x", write_data[i]);
            }
            OS_printf("\n");
        #endif
        if (bytes == GENERIC_MAG_DEVICE_CMD_SIZE)
        {
            status = GENERIC_MAG_ReadData(handle, read_data, GENERIC_MAG_DEVICE_CMD_SIZE);
            if (status == OS_SUCCESS)
            {
                /* Confirm echoed response */
                bytes = 0;
                while ((bytes < (int32_t) GENERIC_MAG_DEVICE_CMD_SIZE) && (status == OS_SUCCESS))
                {
            ...
            } /* GENERIC_MAG_ReadData */
            else
            {
                #ifdef GENERIC_MAG_CFG_DEBUG
                    OS_printf("GENERIC_MAG_CommandDevice - GENERIC_MAG_ReadData returned %d \n", status);
                #endif
            }
        } 
        else
        {
            #ifdef GENERIC_MAG_CFG_DEBUG
                OS_printf("GENERIC_MAG_CommandDevice - uart_write_port returned %d, expected %d \n", bytes, GENERIC_MAG_DEVICE_CMD_SIZE);
            #endif
        } /* uart_write */
    } /* uart_flush*/
    return status;
...
{
    int32_t status = OS_SUCCESS;
    uint8_t read_data[GENERIC_MAG_DEVICE_HK_SIZE] = {0};

    /* Command device to send HK */
    status = GENERIC_MAG_CommandDevice(handle, GENERIC_MAG_DEVICE_REQ_HK_CMD, 0);
    if (status == OS_SUCCESS)
    {
        /* Read HK data */
        status = GENERIC_MAG_ReadData(handle, read_data, sizeof(read_data));
        if (status == OS_SUCCESS)
        {
            #ifdef GENERIC_MAG_CFG_DEBUG
                OS_printf("  GENERIC_MAG_RequestHK = ");
            ...
            if ((read_data[0]  == GENERIC_MAG_DEVICE_HDR_0)     && 
                ...
                (read_data[14] == GENERIC_MAG_DEVICE_TRAILER_0) && 
                (read_data[15] == GENERIC_MAG_DEVICE_TRAILER_1) )
            {
            ...
            {
                #ifdef GENERIC_MAG_CFG_DEBUG
                    OS_printf("  GENERIC_MAG_RequestHK: GENERIC_MAG_ReadData reported error %d \n", status);
                #endif 
                status = OS_ERROR;
            }
        } /* GENERIC_MAG_ReadData */
    ...
    {
        #ifdef GENERIC_MAG_CFG_DEBUG
            OS_printf("  GENERIC_MAG_RequestHK: GENERIC_MAG_CommandDevice reported error %d \n", status);
        #endif 
    }
    return status;
}
...
int32_t GENERIC_MAG_RequestData(int32_t handle, GENERIC_MAG_Device_Data_tlm_t* data)
{
    ...
    /* Command device to send HK */
    status = GENERIC_MAG_CommandDevice(handle, GENERIC_MAG_DEVICE_REQ_DATA_CMD, 0);
    if (status == OS_SUCCESS)
    {
        /* Read HK data */
        status = GENERIC_MAG_ReadData(handle, read_data, sizeof(read_data));
        ...
        {
            #ifdef GENERIC_MAG_CFG_DEBUG
                OS_printf("  GENERIC_MAG_RequestData = ");
                for (uint32_t i = 0; i < sizeof(read_data); i++)
                {
                    OS_printf("%02x", read_data[i]);
                }
            ...
            if ((read_data[0]  == GENERIC_MAG_DEVICE_HDR_0)     && 
                (read_data[1]  == GENERIC_MAG_DEVICE_HDR_1)     && 
                (read_data[12] == GENERIC_MAG_DEVICE_TRAILER_0) && 
                (read_data[13] == GENERIC_MAG_DEVICE_TRAILER_1) )
            {
                data->DeviceCounter  = read_data[2] << 24;
                data->DeviceCounter |= read_data[3] << 16;
                data->DeviceCounter |= read_data[4] << 8;
    ...
    {
        #ifdef GENERIC_MAG_CFG_DEBUG
            OS_printf("  GENERIC_MAG_RequestData: GENERIC_MAG_CommandDevice reported error %d \n", status);
        #endif 
    }
    return status;
}
