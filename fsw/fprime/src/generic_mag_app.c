...
void MAG_AppMain(void) {
  ...
  */
  while (CFE_ES_RunLoop(&GENERIC_MAG_AppData.RunStatus) == true) {
    ...
    */
    status =
        CFE_SB_ReceiveBuffer((CFE_SB_Buffer_t **)&GENERIC_MAG_AppData.MsgPtr,
                             GENERIC_MAG_AppData.CmdPipe, CFE_SB_PEND_FOREVER);
    ...
    */
    if (status == CFE_SUCCESS) {
      ...
      CFE_EVS_SendEvent(GENERIC_MAG_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
                        "GENERIC_MAG: SB Pipe Read Error = %d", (int)status);
      GENERIC_MAG_AppData.RunStatus = CFE_ES_RunStatus_APP_ERROR;
    }
  }
...
int32 GENERIC_MAG_AppInit(void) {
  ...
  */
  ...
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "GENERIC_MAG: Error registering for event services: 0x%08X\n",
        (unsigned int)status);
    return status;
  }
  ...
  */
  status = CFE_SB_CreatePipe(&GENERIC_MAG_AppData.CmdPipe,
                             GENERIC_MAG_PIPE_DEPTH, "MAG_CMD_PIPE");
  if (status != CFE_SUCCESS) {
    CFE_EVS_SendEvent(GENERIC_MAG_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
  ...
  */
  status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(GENERIC_MAG_CMD_MID),
                            GENERIC_MAG_AppData.CmdPipe);
  if (status != CFE_SUCCESS) {
  ...
  */
  status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(GENERIC_MAG_REQ_HK_MID),
                            GENERIC_MAG_AppData.CmdPipe);
  if (status != CFE_SUCCESS) {
  ...
  ** telemetry that has been defined in the GENERIC_MAG_HkTelemetryPkt for this
  *app.
  */
  ...
  *block
  */
  GENERIC_MAG_AppData.HkTelemetryPkt.DeviceEnabled =
      GENERIC_MAG_DEVICE_DISABLED;
  ...
  GENERIC_MAG_AppData.Generic_magSpi.bits_per_word =
      GENERIC_MAG_CFG_BITS_PER_WORD;
  ...
  status = CFE_EVS_SendEvent(
      GENERIC_MAG_STARTUP_INF_EID, CFE_EVS_EventType_INFORMATION,
      "GENERIC_MAG App Initialized. Version %d.%d.%d.%d",
      GENERIC_MAG_MAJOR_VERSION, GENERIC_MAG_MINOR_VERSION,
      GENERIC_MAG_REVISION, GENERIC_MAG_MISSION_REV);
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "GENERIC_MAG: Error sending initialization event: 0x%08X\n",
        (unsigned int)status);
  }
  return status;
}
...
void GENERIC_MAG_ProcessCommandPacket(void) {
  ...
  /*
  ** Ground Commands with command codes fall under the GENERIC_MAG_CMD_MID
  *(Message ID)
  */
  ...
  */
  default:
    ...
    CFE_EVS_SendEvent(GENERIC_MAG_PROCESS_CMD_ERR_EID, CFE_EVS_EventType_ERROR,
                      "GENERIC_MAG: Invalid command packet, MID = 0x%x",
...
void GENERIC_MAG_ProcessGroundCommand(void) {
  ...
  */
  case GENERIC_MAG_NOOP_CC:
    ...
    */
    if (GENERIC_MAG_VerifyCmdLength(GENERIC_MAG_AppData.MsgPtr,
                                    sizeof(GENERIC_MAG_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      /* Second, send EVS event on successful receipt ground commands*/
      CFE_EVS_SendEvent(GENERIC_MAG_CMD_NOOP_INF_EID,
  ...
  */
  case GENERIC_MAG_RESET_COUNTERS_CC:
    if (GENERIC_MAG_VerifyCmdLength(GENERIC_MAG_AppData.MsgPtr,
                                    sizeof(GENERIC_MAG_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      CFE_EVS_SendEvent(GENERIC_MAG_CMD_RESET_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
  ...
  */
  case GENERIC_MAG_ENABLE_CC:
    if (GENERIC_MAG_VerifyCmdLength(GENERIC_MAG_AppData.MsgPtr,
                                    sizeof(GENERIC_MAG_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      CFE_EVS_SendEvent(GENERIC_MAG_CMD_ENABLE_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
  ...
  */
  case GENERIC_MAG_DISABLE_CC:
    if (GENERIC_MAG_VerifyCmdLength(GENERIC_MAG_AppData.MsgPtr,
                                    sizeof(GENERIC_MAG_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      CFE_EVS_SendEvent(GENERIC_MAG_CMD_DISABLE_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
  ...
  */
  default:
    ...
    CFE_EVS_SendEvent(GENERIC_MAG_CMD_ERR_EID, CFE_EVS_EventType_ERROR,
                      "GENERIC_MAG: Invalid command code for packet, MID = "
                      "0x%x, cmdCode = 0x%x",
...
void GENERIC_MAG_ReportHousekeeping(void) {
  ...
  CFE_SB_TransmitMsg((CFE_MSG_Message_t *)&GENERIC_MAG_AppData.HkTelemetryPkt,
                     true);
  return;
}
...
void GENERIC_MAG_ReportDeviceTelemetry(void) {
  ...
  if (GENERIC_MAG_AppData.HkTelemetryPkt.DeviceEnabled ==
      GENERIC_MAG_DEVICE_ENABLED) {
    status = GENERIC_MAG_RequestData(
        &GENERIC_MAG_AppData.Generic_magSpi,
        (GENERIC_MAG_Device_Data_tlm_t *)&GENERIC_MAG_AppData.DevicePkt
            .Generic_mag);
    if (status == OS_SUCCESS) {
      ...
      CFE_SB_TransmitMsg((CFE_MSG_Message_t *)&GENERIC_MAG_AppData.DevicePkt,
                         true);
    } else {
      ...
      CFE_EVS_SendEvent(GENERIC_MAG_REQ_DATA_ERR_EID, CFE_EVS_EventType_ERROR,
                        "GENERIC_MAG: Request device data reported error %d",
                        status);
    }
  }
...
void GENERIC_MAG_Enable(void) {
  ...
  if (GENERIC_MAG_AppData.HkTelemetryPkt.DeviceEnabled ==
    ...
    if (status == OS_SUCCESS) {
      ...
      GENERIC_MAG_AppData.HkTelemetryPkt.DeviceEnabled =
          GENERIC_MAG_DEVICE_ENABLED;
      CFE_EVS_SendEvent(GENERIC_MAG_ENABLE_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
                        "GENERIC_MAG: Device enabled");
    } else {
      GENERIC_MAG_AppData.HkTelemetryPkt.DeviceErrorCount++;
      CFE_EVS_SendEvent(GENERIC_MAG_SPI_INIT_ERR_EID, CFE_EVS_EventType_ERROR,
                        "GENERIC_MAG: SPI port initialization error %d",
                        status);
    }
  } else {
    GENERIC_MAG_AppData.HkTelemetryPkt.DeviceErrorCount++;
...
void GENERIC_MAG_Disable(void) {
  ...
  if (GENERIC_MAG_AppData.HkTelemetryPkt.DeviceEnabled ==
    ...
    if (status == OS_SUCCESS) {
      ...
      GENERIC_MAG_AppData.HkTelemetryPkt.DeviceEnabled =
          GENERIC_MAG_DEVICE_DISABLED;
      CFE_EVS_SendEvent(GENERIC_MAG_DISABLE_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
...
*/
int32 GENERIC_MAG_VerifyCmdLength(CFE_MSG_Message_t *msg,
  ...
  if (expected_length == actual_length) {
    ...
    CFE_EVS_SendEvent(
        GENERIC_MAG_LEN_ERR_EID, CFE_EVS_EventType_ERROR,
